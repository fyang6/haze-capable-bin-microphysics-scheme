...
! fields 
  ...
  units = 'g/m^3'
  call compress3D(tmp,ny,ny,nzm,name,long_name,units, &
  ...
  do m = 1,naerosol
    ...
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          tmp(i,j,k) = Naer(i,j,k,m)*rho(k)*1.0e-6
        end do
      end do
    end do
  ...
  do m = 1,ncloud
    ...
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          tmp(i,j,k) = Ncld(i,j,k,m)*rho(k)*1.0e-6
        end do
      end do
    end do
  ...
  do m = 1,ncloud
    ...
    !print*,rank,'Cloud Mass',m, tmp(1,1,17)
